-- Migration: 014_add_comm_strategy.sql
-- Goal: Allow groups to choose if foreman commission is deducted from Dividend or Payout.

-- 1. Add column to groups
ALTER TABLE public.groups 
ADD COLUMN IF NOT EXISTS commission_strategy text DEFAULT 'deduct_from_payout' 
CHECK (commission_strategy IN ('deduct_from_dividend', 'deduct_from_payout'));

-- 2. Update calculate_auction to respect the strategy
...
BEGIN
  ...
  v_strategy := COALESCE(v_group.commission_strategy, 'deduct_from_payout');
