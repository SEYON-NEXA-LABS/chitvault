...
export default function RegisterPage() {
  ...
  async function checkSlug() {
    ...
    const { data } = await supabase.from('firms').select('id').eq('slug', slug).maybeSingle()
    setLoading(false)
    if (data) { setError('A firm with this name already exists. Try a different name.'); return }
    setStep(2)
  }
  ...
  async function handleRegister() {
    ...
    // 2. Create firm
    const { data: firm, error: firmErr } = await supabase.from('firms').insert({
      name: form.firm_name, slug, owner_id: userId,
  ...
  const inputCls = 'w-full px-4 py-2.5 rounded-lg border text-sm outline-none focus:border-yellow-500'
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: '#0d0f14' }}>
      <div style={{ width: '100%', maxWidth: 460 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: '#c9a84c' }}>Create Your Account</div>
        ...
        <div style={{ background: '#161921', border: '1px solid #2a3045', borderRadius: 16, padding: 28 }}>
          ...
          {step === 1 && (
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>Your Business Details</div>
              ...
              <button onClick={checkSlug} disabled={loading}
                style={{ marginTop: 22, width: '100%', padding: '12px 0', background: '#c9a84c', color: '#0d0f14', borderRadius: 8, border: 'none', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
          ...
          {step === 2 && (
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>Create Your Login</div>
              <div style={{ fontSize: 13, color: '#8892aa', marginBottom: 20 }}>Step 2 of 2 — Set up your admin account</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
                ...
                <button onClick={handleRegister} disabled={loading}
                  style={{ flex: 2, padding: '12px 0', background: '#c9a84c', color: '#0d0f14', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
